...
- (id)init
{
    if ((self = [super init]) != nil)
    {
        ...
        refArray = [NSMutableArray array];
        classDef = [NSMutableArray array];
        typeMap = [NSMutableArray array];
        objectDefinitionMap = [NSMutableArray array];
        fieldsByTypeName = [NSMutableDictionary dictionary];
    }
    return self;
}
...
{
    BBSHessianDecoder *decoder = [[BBSHessianDecoder alloc] initForReadingWithData:someData];
...
- (id)initForReadingWithData:(NSData *)someData
{
    ...
    {
        dataInputStream = [NSInputStream inputStreamWithData:someData];
...
- (id)decodeObjectForCode:(uint8_t)code
{
    ...
    {
    case 'b':
        obj = [self decodeByteChunks];
        break;
    case 'B':
        obj = [self decodeBytes];
        break;
    case 'T':
    ...
    case 'd':
        ...
        break;
    case 'D':
        obj = [self decodeDouble];
        break;
    case 'f':
        obj = [self decodeFault];
        break;
    case 'I':
        obj = [self decodeIntForCode:code];
        break;
    case 'v':
        obj = [self decodeList:code];
        break;
    case 'V':
        obj = [self decodeList:code];
        break;
    ...
    case 'N':
        ...
        break;
    case 'o':
        obj = [self decodeCompactObject];
        break;
    case 'O':
        [self decodeCompactObjectHeader];
        obj = [self decodeObject];
        break;
    case 'R':
        obj = [self decodeRef:code];
        break;
    case 0x4a:
        obj = [self decodeRef:code];
        break;
    case 0x4b:
        ...
        break;
    case 's':
        obj = [self decodeString:code];
        break;
    case 'S':
        obj = [self decodeString:code];
        break;
    case 'x':
        obj = [self decodeXml:'x'];
        break;
    case 'X':
        obj = [self decodeXml:'X'];
        break;
    default:
    {
        ...
        NSDictionary *userInfo = [NSDictionary
            dictionaryWithObject:[NSString stringWithFormat:@"Unknown tag returned %c", code]
...
- (id)allocObjectWithType:(NSMutableDictionary *)dict andType:(NSString *)type
{
    ...
    {
        NSString *localClass =
            [type stringByReplacingOccurrencesOfString:remoteClassPrefix ?: @"" withString:@""];
...
- (id)decodeMap
{
    ...
    if ([dataInputStream hasBytesAvailable])
    {
        ...
        if (val)
        {
            ...
            val = [val initWithCoder:mapDecoder];
            return val;
        }
        else
        {
...
- (NSError *)decodeFault
{
    ...
    if (objectTag != 'S')
    {
        NSDictionary *userInfo =
            [NSDictionary dictionaryWithObject:@"Malformed fault" forKey:NSLocalizedDescriptionKey];
...
#pragma unused(detailProp)
    ...
    // this causes problems when callign descriptions on the fault dictionary because it causes an
    // infinit loop
    ...
    NSMutableDictionary *userInfo = [NSMutableDictionary
        dictionaryWithObject:[NSString stringWithFormat:@"%@:%@", codeMessage, message]
                      forKey:NSLocalizedDescriptionKey];
...
- (void)addToRefArray:(id)val
{
    [refArray addObject:[NSValue valueWithPointer:(__bridge const void *)(val)]];
}
