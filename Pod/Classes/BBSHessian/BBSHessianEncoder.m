...
- (void)encodeNSObject:(NSObject *)anObject
{
    ...
    if ([anObject conformsToProtocol:@protocol(NSCoding)])
    {
        ...
        {
            className = [NSString
                stringWithFormat:@"%@%@", remoteClassPrefix, NSStringFromClass([anObject class])];
        }
...
- (void)encodeString:(NSString *)aString
{
    ...
    char s = 's';
    char S = 'S';
    while (currentLen > 0xFFFF)
    { /* 32768 or 0x8000 is the 16 bit length limit for a binary chunk */
